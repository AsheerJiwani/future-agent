...
/* --- Football panel with toggles & computed decision --- */
function FootballPanel() {
  const concept = SMASH_CONCEPT;

  const COVERAGE_OPTIONS: CoverageID[] = ["C2","TAMPA2","PALMS","C3","C4","QUARTERS"];

  const [coverage, setCoverage] = useState<CoverageID>(concept.diagram?.coverage ?? "C2");
  const [rotateStrong, setRotateStrong] = useState<boolean>(false);
  const [nickelBlitz, setNickelBlitz] = useState<boolean>(false);
  const [explain, setExplain] = useState<string>("");

  const plan = useMemo<ReadPlan>(() => {
    const p = concept.readPlans.find(r => r.vs === coverage);
    return p ?? concept.readPlans[0];
  }, [concept, coverage]);
  ...
  function computeDecision(cov: CoverageID, opts: { rotateStrong: boolean; nickelBlitz: boolean }) {
    ...
    if (opts.nickelBlitz) {
      ...
      highlight = "Z";
      activeStep = 2;
      decision = "Base vs Cover 2: read cloud corner then flat; if flat widens, hit the corner to Z at 18–22.";
    } else if (cov === "PALMS") {
      highlight = "H";
      activeStep = 1;
  ...
  async function getExplanation() {
    try {
      ...
      // fallback: keep deterministic string so UI still shows something
      setExplain("Smash vs " + coverage + ": key cloud corner → hitch now if he sinks; if flat widens, throw corner. Hot: replace nickel blitz with hitch.");
    }
  }

  const diagram = useMemo<Diagram>(() => {
    const d = concept.diagram!;
    return { ...d, coverage };
  }, [concept, coverage]);
    ...
    <div className="panel-surface thin-border" style={{ background: "var(--surface)" }}>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <label className="text-sm opacity-80">Coverage:</label>
          <select
            value={coverage}
          ...
          <button onClick={getExplanation} className="btn-accent ml-2">Explain this read</button>
        </div>
      </div>

      <div className="text-white/70 text-sm mb-2">
        Concept: <span className="font-medium">{concept.name}</span> · Footwork: <span className="font-medium">{concept.footwork || "—"}</span>
      </div>

      <PlayDiagram diagram={diagram} highlightLabel={highlight} />

      <div className="grid md:grid-cols-2 gap-4 mt-4">
        <ReadStepper plan={plan} activeIndex={activeStep} />
        <div className="rounded-2xl thin-border p-4 bg-white/5">
